...
@group(0) @binding(4) var<storage, read> signed_counts: array<i32>;
...
fn reset_decay(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let arrival = atomicExchange(&arrivals[neuron], 0);
...
fn propagate(@builtin(global_invocation_id) global_id: vec3<u32>) {
  let source = global_id.x;
  if (source >= tick.neuron_count || states[source].silenced != 0u) {
    return;
  }
  let fired = select(
    spike_ring[tick.ring_slot * tick.neuron_count + source],
    states[source].spikes,
    tick.delay_steps == 0u,
  );
  if (fired == 0u) {
    return;
  }
  let begin = row_ptr[source];
  let end = row_ptr[source + 1u];
  for (var edge = begin; edge < end; edge += 1u) {
    atomicAdd(&arrivals[destinations[edge]], signed_counts[edge]);
  }
}

@compute @workgroup_size(256)
fn apply_external_sparse(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let lane = packed & 0x00ffffffu;
  let count = packed >> 24u;
  let destination = events[lane];
  states[destination].voltage += f32(count) * tick.external_weight_mv;
}
