...
fn propagate(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let count = u32(atomicLoad(&arrivals[tick.neuron_count + tick.active_slot]));
  for (var index = global_id.x / SOURCE_LANES; index < count; index += MAX_SOURCE_GROUPS) {
    ...
    for (var edge = begin + global_id.x % SOURCE_LANES; edge < end; edge += SOURCE_LANES) {
